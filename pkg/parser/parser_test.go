...
func TestParseEmptyBlock(t *testing.T) {
	cases := []struct {
		...
		{10, "@if(true)@end", 12, token.IF},
		{20, "@if(true)1@else@end", 18, token.IF},
		{30, "@if(true)@else@end", 17, token.IF},
		{40, "@if(true)@else1@end", 18, token.IF},
		{50, "@each(name in ['anna', 'serhii'])@end", 36, token.EACH},
		{60, "@each(name in ['anna', 'serhii'])@else@end", 41, token.EACH},
		{70, "@each(name in ['anna', 'serhii'])1@else@end", 42, token.EACH},
		{80, "@each(name in ['anna', 'serhii'])@else1@end", 42, token.EACH},
		{90, "@for(i = 0; i < 10; i++)@end", 27, token.FOR},
		{100, "@for(i = 0; i < 10; i++)@else@end", 32, token.FOR},
		{110, "@for(i = 0; i < 10; i++)1@else@end", 33, token.FOR},
		{120, "@for(i = 0; i < 10; i++)@else1@end", 33, token.FOR},
		{130, "@insert('content')@end", 21, token.INSERT},
		{140, "@component('user')@slot@end@end", 30, token.COMPONENT},
		{140, "@component('user')@slot('footer')@end@end", 40, token.COMPONENT},
		{150, "@component('user')@slotif(true, 'footer')@end@end", 48, token.COMPONENT},
		{150, "@component('user')@slotif(true)@end@end", 38, token.COMPONENT},
	}
