...
func TestEvalStringFunctions(t *testing.T) {
	cases := []struct {
		...
		{`{{ "".last() }}`, ""},
		// format
		{`{{ "He has %s apples".format(2) }}`, "He has 2 apples"},
		{`{{ "First: %s. Last: %s".format('Amy', 'Adams') }}`, "First: Amy. Last: Adams"},
		{`{{ "%s-%s-%s".format(0.1, false, true) }}`, "0.1-0-1"},
		{`{{ "[%s]".format([1, 2]) }}`, "[1, 2]"},
		{`{{ "%s-%d".format("nice") }}`, "nice-%d"},
		{`{{ "%%s".format("Sydney") }}`, "%Sydney"},
		{`{{ "|%s and %s|".format("Anna") }}`, "|Anna and %s|"},
		{`{{ "".format("ignored") }}`, ""},
		{`{{ "Hello World".format("extra") }}`, "Hello World"},
		{`{{ "Only one: %s".format("first", "second", "third") }}`, "Only one: first"},
		{`{{ "%s%s%s".format("a", "b", "c") }}`, "abc"},
		{`{{ "50%% complete".format("ignored") }}`, "50%% complete"},
		{`{{ "This %s is nice".format('%s') }}`, "This %s is nice"},
		{`{{ "%s".format(42) }}`, "42"},
		{`{{ "%s".format(-99) }}`, "-99"},
		{`{{ "Value: %s".format(true) }}`, "Value: 1"},
		{`{{ "Value: %s".format(false) }}`, "Value: 0"},
